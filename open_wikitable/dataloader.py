...
class OpenWikiTableFull(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        original_table_id = self.data.iloc[idx].original_table_id
...
def tokenize_tables(tables : pd.DataFrame, config):
    ...
    if "bert" in table_encoder or 'sentence' in table_encoder :

        table_encoder_name = "bert" if "bert" in table_encoder else 'sentence'
...
def get_retriever_dataloader(tokenizer, config):
    ...
    elif 'sentence' in table_encoder:
        table_encoder_name = "sentence"
    else:
        table_encoder_name = "tapas"
...
class tokenized_table(Dataset):
    def __init__(self, config):
        ...
        elif 'sentence' in config["table_encoder"]:
            path = os.path.join(config["path"],"augmented","sentence_table_tokenized.pkl")
        else:
...
def get_table_dataloader(config):
    ...
    table_dataloader = DataLoader(data, batch_size = 128, shuffle= False ,num_workers =4)

    return table_dataloader



def test_get_dataloader(tokenizer, config):
    ...
    question_tokenized_lens = []
    for sample in train_dataset :
        question_tokenized = tokenizer(sample['question'], return_tensors = "pt")
        question_tokenized_lens.append(question_tokenized['input_ids'].shape[-1])

    question_tokenized_lens = torch.tensor(question_tokenized_lens)
...
if __name__ == '__main__' :
    ...
    # with open('/scratch/project/Open_WikiTable-main/config/retriever/reader_bert_bert.yaml', 'r') as f:
    #     config = yaml.load(f, Loader = yaml.FullLoader)
    # question_tokenizer = AutoTokenizer.from_pretrained(config["question_encoder"])
    # test_get_dataloader(question_tokenizer, config)

    # tokenize and save tables
    with open('./config/retriever/reader_bert_bert_test.yaml', 'r') as f:
        config = yaml.load(f, Loader = yaml.FullLoader)
    tokenize_tables(pd.read_json(os.path.join(config["path"],"splitted_tables.json")), config)